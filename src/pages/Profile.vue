...
<template>
  <PageHeader :title="translate('Profile')" />
  <form @submit.prevent="save" class="container-app max-w-3xl py-7">
    <div class="flex items-center gap-4">
      ...
      >
      <div>
        <h1 class="text-2xl font-semibold">{{ translate("Tadej Boncelj") }}</h1>
        <p class="mt-1 text-sm text-muted">{{ translate("Vintro Free · 2 vehicles") }}</p>
      </div>
    </div>
    <section class="card mt-8 p-5 md:p-6">
      <div class="flex items-center gap-2">
        <UserRound :size="19" />
        <h2 class="section-title">{{ translate("Contact information") }}</h2>
      </div>
      <div class="mt-5 grid gap-5 sm:grid-cols-2">
        <label
          ><span class="label">{{ translate("Full name") }}</span
    ...
    <section class="card mt-5 p-5 md:p-6">
      <div class="flex items-center gap-2">
        <Languages :size="19" />
        <h2 class="section-title">{{ translate("Application language") }}</h2>
      </div>
      <p class="mt-2 text-sm text-muted">{{ translate("Choose the language used throughout Vintro.") }}</p>
    ...
    <section class="card mt-5 p-5 md:p-6">
      <div class="flex items-center gap-2">
        <Bell :size="19" />
        <h2 class="section-title">{{ translate("Notification preferences") }}</h2>
      </div>
      <div class="mt-5 divide-y">
        <label
          ...
          ><span
            ><strong class="block text-sm">{{ translate((item.title)) }}</strong
            ><small class="mt-1 block text-muted">{{ translate((item.text)) }}</small></span
          ><input
      ...
      <label class="mt-5 flex gap-3 text-sm text-muted"
        ...
        />{{ translate("I agree to receive occasional product news and partner offers.") }}</label
      >
    </section>
    <div class="mt-5 grid gap-3 sm:grid-cols-3">
      <RouterLink to="/membership" class="card p-4"
        ><strong class="block text-sm">{{ translate("Vintro Plus") }}</strong
        ><small class="text-muted">{{ translate("Plans & loyalty") }}</small></RouterLink
      ><RouterLink to="/insurance" class="card p-4"
        ><strong class="block text-sm">{{ translate("Insurance") }}</strong
        ><small class="text-muted">{{ translate("Policies & reminders") }}</small></RouterLink
      ><RouterLink to="/accept-transfer" class="card p-4"
        ><strong class="block text-sm">{{ translate("Receive vehicle") }}</strong
    ...
    ><RouterLink
      ...
      ><LogOut :size="17" />{{ translate("Log out") }}</RouterLink
    >
  </form>
</template>
