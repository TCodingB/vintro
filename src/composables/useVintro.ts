...
const state=reactive({vehicles:[...vehicles],primaryVehicleId:'bmw-320d',services:[...initialServices],vehicleEvents:[...initialVehicleEvents],documents:[...initialDocuments],quotes:[...initialQuotes],bookings:[{id:'bmw-tyres-2026-09-01',workshop:'Servis Ljubljana',vehicleId:'bmw-320d',vehicle:'320d Touring',service:'Tyres',slot:'Tue 1 Sep · 08:00',note:'',status:'Requested'}] as {id:string;vehicleId:string;workshop:string;vehicle:string;service:string;slot:string;note:string;status:string}[],notifications:[{id:'n1',title:'Technical inspection overdue',text:'BMW 320d Touring · due 18 August 2026',kind:'Maintenance',read:false},{id:'n2',title:'Quote received',text:'Servis Ljubljana sent a € 600 quote',kind:'Quote',read:false},{id:'n3',title:'Insurance expires soon',text:'Policy expires 31 December 2026',kind:'Document',read:true}],signedIn:true,toast:''})
...
export function useVintro(){
 ...
 const addBooking=(booking:{id:string;vehicleId:string;workshop:string;vehicle:string;service:string;slot:string;note:string;status:string})=>{state.bookings.unshift(booking);notify('Booking request sent')}
